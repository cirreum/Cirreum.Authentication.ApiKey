...
- **Tests** — new `Cirreum.Authentication.ApiKey.Tests` (82 tests): constant-time compare, fail-closed hash dispatch, two-forms strength, revocation gate + denylist, source dispatch (config-first, authoritative routing, the client-id gate), the handler's fail-closed posture and status mapping, and input fuzzing.
